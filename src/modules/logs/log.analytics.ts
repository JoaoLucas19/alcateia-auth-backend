...
export function buildTimeline24h(
  adminLogs: { createdAt: Date; success: boolean }[],
  clientLogs: { createdAt: Date; success: boolean }[],
  keyLogs: { attemptedAt: Date; result: string }[]
): TimelineHourBucket[] {
  const buckets = new Map<string, TimelineHourBucket>();

  const ensure = (date: Date): TimelineHourBucket => {
  ...
  for (const log of adminLogs) {
    ...
    if (log.success) b.adminSuccess++;
    else b.adminFailed++;
  }

  for (const log of clientLogs) {
    const b = ensure(log.createdAt);
    if (log.success) b.clientSuccess++;
    else b.clientFailed++;
  }
...
export function aggregateSuspiciousIps(input: {
  adminFailures: { ipAddress: string; createdAt: Date }[];
  clientFailures: { ipAddress: string; createdAt: Date }[];
  invalidKeyByIp: { ipAddress: string; _count: number }[];
}): SuspiciousIpEntry[] {
  const map = new Map<string, IpAggregate>();

  const touch = (ip: string, at: Date) => {
    if (!map.has(ip)) {
...
export function detectSecurityAlerts(input: {
  ...
  adminFailuresByIp: { ipAddress: string; _count: number }[];
...
export function computeThreatLevel(alerts: SecurityAlert[], suspiciousIps: SuspiciousIpEntry[]): {
  level: ThreatLevel;
  score: number;
} {
  let score = 0;
